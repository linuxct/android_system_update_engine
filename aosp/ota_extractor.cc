...
void ExtractImageFromPartitions(const DeltaArchiveManifest& manifest,
  ...
  for (const auto& op : partition.operations()) {
    ...
    TEST_AND_RETURN(utils::PReadAll(
    ...
    if (op.type() == InstallOperation::ZERO) {
      TEST_AND_RETURN(executor.ExecuteZeroOrDiscardOperation(
      ...
      TEST_AND_RETURN(executor.ExecuteReplaceOperation(
          op, std::move(direct_writer), blob.data(), blob.size()));
    } else if (op.type() == InstallOperation::SOURCE_COPY) {
      CHECK(in_fd->IsOpen());
      TEST_AND_RETURN(executor.ExecuteSourceCopyOperation(
          op, std::move(direct_writer), in_fd));
    } else {
      CHECK(in_fd->IsOpen());
      TEST_AND_RETURN(executor.ExecuteDiffOperation(
